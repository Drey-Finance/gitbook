...
Technically, a zero knowledge proof encapsulates the following characteristics:
...
1. **Identity**: Users can prove, in zero knowledge, that they possess a key issued to a certain anonymized identity encapsulating actuarial data without revealing the key or the identity.
...
While neither a cryptographic or communications protocol, Bitcoin's recursive inscriptions are important to Drey Finance's architecture. Developers have introduced the idea of calling data from existing inscriptions and using that data within new inscriptions, a concept known as a 'recursive inscription'. This allows robust software to run entirely on-chain.
...
3. **Trustless Off-Chain Smart Contracts**: By combining these two technologies, it's possible to create trustless off-chain smart contracts on the Bitcoin network.
...
Drey Actuaries respond to a cryptographically secured request from the Drey Proof of Life App to generate a share of this identity based private key and encrypt it with a public key generated in the Drey Proof of Life App operated by the Drey customer. The public key encryption is necessary to secure the share so that no party, including any intermediary relay, can obtain access to the share. Drey Actuaries will also provide shares of a server secret to other Drey Actuary clients so individually each will be enabled to run the [M-Pin zero-knowledge proof multi-factor identity verification protocol ](https://milagro.apache.org/docs/milagro-protocols)with Drey customers executing the Proof of Life operation every month. Note the Drey Actuaries do not need to co-ordinate with each other to issue shares, a DTA simply responds to the request from the Drey Proof of Life App or other entitled Drey Actuary clients for a share.
