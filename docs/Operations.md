...
A Drey Actuary (miner), selected by verifiable random function, proposes a monthly distribution schedule called a Dreybit ledger based upon the R software packages each Drey Actuary (miner) runs to operate the fund. This proposed distribution schedule is written into the bitcoin blockchain at a specific satoshi and relayed to the Drey Actuary (miner) population over nostr. The decentralised actuary protocol summons a vote by creating a transaction at _x_ block height to move the satoshi to a specific wallet address conveying acceptance or rejection. Incentives for good behaviour and disincentives for bad behaviour are discussed in the [**Drey Economics**](dreyconomics/stakeholders.md).
...
Each Dreybit ledger contains an anonymized ID created for the user during signup which maps to their bitcoin address and Dreybits allocated. The amount of Dreybits allocated depends on the investor’s probability of dying and amount invested (explained in a following section) relative to all investments in a particular fund.
